...
class UserSerializerTests(APITestCase):
    ...
    def test_create_user_with_invalid_group(self):
        ...
        self.assertIn('Los siguientes grupos no existen: nonexistent_group', serializer.errors['groups'][0])
    ...
    def test_update_user_add_groups(self):
        user = User.objects.create_user(username='testuser4', password='password123') #Crear usuario sin grupos.
        data = {
            'groups': ['group1', 'group3'],
        }
        serializer = UserSerializer(instance=user, data=data, partial=True) #partial=True para permitir update parcial
        self.assertTrue(serializer.is_valid())
        user = serializer.save()
        self.assertEqual(user.groups.count(), 2)
        self.assertIn(self.group1, user.groups.all())
    ...
    def test_update_user_remove_all_groups(self):
        user = User.objects.create_user(username='testuser5', password='password123')
    ...
    def test_update_user_with_no_groups_in_data(self):
        user = User.objects.create_user(username='testuser6', password='password123')
        user.groups.add(self.group1)  # Añade un grupo inicialmente
        data = {
        ...
        self.assertTrue(serializer.is_valid())
        user = serializer.save()
        self.assertEqual(user.email, 'new_email@example.com') #Comprueba que se actualizo el email.
        self.assertEqual(user.groups.count(), 1)  # No debe haber cambiado los grupos.
        self.assertIn(self.group1, user.groups.all())  # Debe mantener el grupo anterior.


    def test_update_user_with_invalid_group(self):
        user = User.objects.create_user(username='testuser7', password='password123')
        data = {
            'groups': ['group1', 'nonexistent_group'],
        }
        serializer = UserSerializer(instance=user, data=data, partial=True)
